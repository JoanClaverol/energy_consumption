...
df2 %>% 
  ...
  labs(title = "Mean active energy consumed by day") + ylab("Watt/h") + xlab("Time") + theme_light() -> g1
ggplotly(g1)
...
df2 %>% 
  mutate(wday = wday(DateTime, label = T, abbr = F, locale = "us", week_start = 1), 
         hour.minutes = paste(hour(DateTime), minute(DateTime), second(DateTime), sep = ":")) %>% 
  group_by(wday, hour.minutes) %>% summarise(median = round(median(ActiveEnergy),2)) -> week.general

week.general$hour.minutes <- as.POSIXct(strptime(week.general$hour.minutes, format = "%H:%M:%S", tz = "GMT"), format = "%H:%M:%S")

week.general %>% 
  ggplot(aes(hour.minutes, median)) + geom_line() + facet_wrap(wday~.) + 
  labs(title = "Median active energy/week") + ylab("Median Watt/h") + xlab("Time") + theme_light() + scale_x_datetime(date_labels = "%H:%M") -> g2
ggplotly(g2)
...
week.general %>% 
  ggplot(aes(wday, median)) + geom_boxplot() # No outliers
...
df2 %>% 
  select(season, x) %>% gather(x, key = submeters, value = energy_consumption) %>% 
